...
import type { CandleData, MarketMode } from '../types';
  ...
  return (
    <div className="w-80 shrink-0 flex flex-col bg-terminal-panel border border-terminal-border min-h-0">
      ...
      <div className="px-2 py-1 border-b border-[#1a1a1a] shrink-0 font-mono text-[9px] flex gap-3 items-center">
        <span className={sector.avgChange >= 0 ? 'text-terminal-success font-bold' : 'text-terminal-error font-bold'}>
          {sector.avgChange >= 0 ? '+' : ''}{sector.avgChange.toFixed(2)}%
        </span>
        <span className="text-gray-600">|</span>
        <span className="text-terminal-success">{sector.advancing}↑</span>
        <span className="text-terminal-error">{sector.declining}↓</span>
        <span className="text-gray-600">|</span>
        ...
        {chartType === 'RELATIVE' ? (
          loading ? (
            ...
            <div className="flex items-center justify-center h-full text-gray-700 text-[9px] font-mono">
              NO DATA — SYMBOLS MAY NOT MATCH MARKET MODE
            </div>
          ) : (
            ...
            >
              ...
              <Tooltip
                ...
                formatter={(value) => {
                  const v = Number(value ?? 0);
                  return [
                    <span key="v" style={{ color: v >= 0 ? '#00cc66' : '#ff3333', fontWeight: 700 }}>
                      {v >= 0 ? '+' : ''}{v}%
                    </span>,
                    'CHG',
                  ];
